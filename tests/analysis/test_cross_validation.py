...
from analysis.cross_validation import _fit_and_predict
from core import ConfidenceInterval, IntervalKind, Settings, TrainingData, select_split
from core.schemas import SplitDatasetBase, SplitKind
...
def test_confidence_intervals_is_deterministic(
    ...
    deterministic_cv_settings: Settings,
) -> None:
    first = confidence_intervals(unfitted_pipeline, split_dataset, deterministic_cv_settings)
    second = confidence_intervals(unfitted_pipeline, split_dataset, deterministic_cv_settings)
...
def test_confidence_intervals_interval_width_positive(
    split_dataset: DataFrame[SplitDatasetBase],
    unfitted_pipeline: Pipeline,
    positive_width_cv_settings: Settings,
) -> None:
    intervals = confidence_intervals(unfitted_pipeline, split_dataset, positive_width_cv_settings)
...
def test_confidence_intervals_uses_settings_defaults(
    split_dataset: DataFrame[SplitDatasetBase],
    unfitted_pipeline: Pipeline,
    small_cv_settings: Settings,
) -> None:
    intervals = confidence_intervals(unfitted_pipeline, split_dataset, small_cv_settings)
...
def test_confidence_intervals_width_varies_with_x(
    split_dataset: DataFrame[SplitDatasetBase],
    unfitted_pipeline: Pipeline,
    varying_width_cv_settings: Settings,
) -> None:
    intervals = confidence_intervals(unfitted_pipeline, split_dataset, varying_width_cv_settings)
    widths = intervals[ConfidenceInterval.upper] - intervals[ConfidenceInterval.lower]
...
def test_confidence_intervals_accepts_fitted_template(
    ...
    minimal_cv_settings: Settings,
) -> None:
    intervals = confidence_intervals(fitted_pipeline, split_dataset, minimal_cv_settings)
...
def test_confidence_intervals_empirical_coverage_against_mu(
    split_dataset: DataFrame[SplitDatasetBase],
    unfitted_pipeline: Pipeline,
    empirical_coverage_cv_settings: Settings,
) -> None:
    intervals = confidence_intervals(
        unfitted_pipeline,
        split_dataset,
        empirical_coverage_cv_settings,
    )
    eval_split = select_split(split_dataset, SplitKind.EVALUATION)
    mu_true = mean_function(eval_split[TrainingData.x].to_numpy(), empirical_coverage_cv_settings)
...
def test_per_fold_seed_changes_predictions(
    split_dataset: DataFrame[SplitDatasetBase],
    unfitted_pipeline: Pipeline,
) -> None:
    train = select_split(split_dataset, SplitKind.TRAINING)
    eval_data = select_split(split_dataset, SplitKind.EVALUATION)
    preds_seed_a = _fit_and_predict(unfitted_pipeline, train, eval_data, seed=1)
    preds_seed_b = _fit_and_predict(unfitted_pipeline, train, eval_data, seed=99)
